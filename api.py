...
# 1. Modeli ve Orijinal Veriyi Yükle
...
# 2. Şablonu "Metin" (String) Olarak Bırakıyoruz!
# ASLA le.fit_transform() YAPMIYORUZ. CatBoost'un kendi içindeki encoder'ı çalışsın.
X_clean[kategorik_sutunlar] = X_clean[kategorik_sutunlar].fillna("Unknown")
X_clean = X_clean.fillna(0)

# Şablonu, sütunların orijinal tipinde kopyalıyoruz
...
def risk_tahmin_et(veri: dict):
    ...
    # 3. KELİMELERİ DİREKT YAPIŞTIR (Çeviri yok!)
    mode_map = {
    ...
    # Değerleri direkt yerleştiriyoruz. Sayıya çevirmiyoruz.
    test_verisi["Order City"] = veri.get("Order_City", X_sablon["Order City"].values[0])
    test_verisi["Category Id"] = veri.get("Category_Id", X_sablon["Category Id"].values[0])
    test_verisi["Shipping Mode"] = orijinal_mod_metni

    # 4. RİSK HESAPLAMA MOTORU
    def calculate_base_risk(df_input, w_desc):
        # DİKKAT: astype(float) YOK! Model metin bekliyor, metin veriyoruz.
        try:
    ...
    # 5. Lojistik Danışmanı (Tavsiye Motoru)
    all_modes = {
    ...
    for display_name, mode_str in all_modes.items():
        if mode_str != gelen_mod_str:
            ...
            alt_test["Shipping Mode"] = mode_map.get(mode_str, "Standard Class")
